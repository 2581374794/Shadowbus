...
战斗阶段采用 Host 权威模式：访客只发送出牌、进化、融合、攻击、结束回合及选择等操作请求；房主使用原版战斗管理器执行请求，再把原版形状的 `PlayActions`、回合转换和结果包发给访客回放。开局会交换 Host 所需的私有卡牌/状态基线，之后只随权威结果发送紧凑的状态变化。此模式用于可信的好友对战，不提供反作弊保证。
