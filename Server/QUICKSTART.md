...
默认监听端口为 `29600`，默认绑定地址为 `0.0.0.0`。房主未填写
`AdvertisedAddress` 时，插件会优先自动选择活动的虚拟网卡地址（例如 Radmin、
Hamachi、ZeroTier、Tailscale、WireGuard），再选择普通局域网地址。跨机器联机时，
房间码中的地址必须是加入者可以访问的地址，不能使用 `127.0.0.1`。

如果自动选择的地址不是朋友实际使用的虚拟网卡地址，请在
`BepInEx/config/08c8e386-a794-442f-a98c-aec65a183898.cfg` 的 `[SocketIO]` 段显式设置：

```ini
BindAddress = 0.0.0.0
AdvertisedAddress = 26.x.x.x
Port = 29600
```

其中 `26.x.x.x` 仅为 Radmin 示例，实际填写房主虚拟网卡的 IPv4 地址。还需要在
Windows 防火墙中允许入站 TCP `29600`；虚拟局域网软件本身也必须显示双方在线且可互相 ping。
