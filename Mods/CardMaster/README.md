...
当前 Shadowbus 的 `newCard` 在创建普通卡时会**自动派生一条配套的闪卡记录**，无需手写第二条。派生规则：

- 闪卡 ID 取普通版 ID `+1`，例如普通版 `999991000` 会得到闪卡 `999991001`。
- 闪卡克隆自**模板卡自己的闪卡版本**（`templateCardId` 的 `FoilCardId`），因此会一并继承闪卡材质与卡图效果。
- 闪卡的属性、技能与文本与该补丁完全一致。
- 未填写 `BaseCardId/NormalCardId` 时，new card 会按当前补丁逻辑使用新卡自身 ID 作为默认值。

以下情况**不会**自动生成闪卡：

- 补丁里显式写了 `"IsFoil": true` —— 说明你在手工配对普通版与闪卡版，插件不介入。
- 闪卡 ID（普通版 `+1`）已被占用，或已被同一批次的其他补丁声明。此时日志会输出 `gets no foil companion: card id ... is already claimed`。

因此自制卡请成对预留 ID：`999991000` 配 `999991001`，`999991010` 配 `999991011`。若想完全手工控制两条记录，就显式写 `IsFoil` 并按第五节的写法配对。
...
2. 普通版和闪卡版使用相邻 ID，例如 `999991000` 和 `999991001`。自动配套功能正是按「普通版 `+1`」派生闪卡 ID 的，所以请按这个约定为每张自制卡预留成对的 ID。手工配对（显式写 `IsFoil`）时，关系字段必须显式填写，不要只依赖“末位加一”的猜测。
...
> 这是**手工配对**写法：闪卡记录带了 `"IsFoil": true`，因此插件不会再自动派生闪卡。
> 只想写普通版时，删掉第二条即可 —— 插件会自动按 ID `+1` 生成闪卡。
